...
**Estado real:** 🟡 `building` — hay código y **4 comprobaciones automáticas**, sin levantarse bajo `bwrap` en CI · **Carpeta:** [`cases/06-microvm-detonation/`](../../cases/06-microvm-detonation) · **Puerto:** `8806`
...
ellas**, no cómo está escrito. Son 4 comprobaciones, y corren en cada
commit.

```bash
cargo run -p sandboxctl -- service up microvm-detonation
```

Levanta el caso como producto en `127.0.0.1:8806`. `POST /api/run` acepta el
