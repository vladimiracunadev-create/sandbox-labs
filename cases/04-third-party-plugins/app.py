#!/usr/bin/env python3
"""Servicio del caso 04 — Plugins de terceros.
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path in ("/health", "/api/status"):
            self._json(200, {"status": "ok", "case": "04", "runtime": RUNTIME})
            return
        if self.path == "/":
            self._json(200, {
                "case": "04",
                "title": "Plugins de terceros",
                "teaches": "Sumar capacidades en vez de restar permisos. El punto de partida no es «todo menos lo prohibido», es nada.",
                "runtime": RUNTIME,
...
def main() -> None:
    if SOCKET_PATH:
        ...
        with UnixServer(SOCKET_PATH, Handler) as server:
            ...
            print(f"servicio 04-third-party-plugins en unix:{SOCKET_PATH} · runtime={RUNTIME} · sin red", flush=True)
            server.serve_forever()
        return

    socketserver.TCPServer.allow_reuse_address = True
    with socketserver.TCPServer(("127.0.0.1", PORT), Handler) as server:
        print(f"servicio 04-third-party-plugins en 127.0.0.1:{PORT} · runtime={RUNTIME}", flush=True)
        server.serve_forever()
