#!/usr/bin/env python3
"""Servicio del caso 12 — Notebooks de ciencia de datos.
...
PORT = int(os.environ.get("SANDBOX_PORT", "8812"))
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path in ("/health", "/api/status"):
            self._json(200, {"status": "ok", "case": "12", "runtime": RUNTIME})
            return
        if self.path == "/":
            self._json(200, {
                "case": "12",
                "title": "Notebooks de ciencia de datos",
                "teaches": "Aquí lo valioso son los datos: protegerlos es que el análisis no pueda modificar aquello que analiza.",
                "runtime": RUNTIME,
...
def main() -> None:
    if SOCKET_PATH:
        ...
        with UnixServer(SOCKET_PATH, Handler) as server:
            ...
            print(f"servicio 12-notebook-sandbox en unix:{SOCKET_PATH} · runtime={RUNTIME} · sin red", flush=True)
            server.serve_forever()
        return

    socketserver.TCPServer.allow_reuse_address = True
    with socketserver.TCPServer(("127.0.0.1", PORT), Handler) as server:
        print(f"servicio 12-notebook-sandbox en 127.0.0.1:{PORT} · runtime={RUNTIME}", flush=True)
        server.serve_forever()
