#!/usr/bin/env python3
"""Servicio del caso 06 — Detonación en microVM.
...
PORT = int(os.environ.get("SANDBOX_PORT", "8806"))
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path in ("/health", "/api/status"):
            self._json(200, {"status": "ok", "case": "06", "runtime": RUNTIME})
            return
        if self.path == "/":
            self._json(200, {
                "case": "06",
                "title": "Detonación en microVM",
                "teaches": "Un namespace comparte kernel. Para observar algo que intenta escapar, la frontera tiene que ser una máquina.",
                "runtime": RUNTIME,
...
def main() -> None:
    if SOCKET_PATH:
        ...
        with UnixServer(SOCKET_PATH, Handler) as server:
            ...
            print(f"servicio 06-microvm-detonation en unix:{SOCKET_PATH} · runtime={RUNTIME} · sin red", flush=True)
            server.serve_forever()
        return

    socketserver.TCPServer.allow_reuse_address = True
    with socketserver.TCPServer(("127.0.0.1", PORT), Handler) as server:
        print(f"servicio 06-microvm-detonation en 127.0.0.1:{PORT} · runtime={RUNTIME}", flush=True)
        server.serve_forever()
