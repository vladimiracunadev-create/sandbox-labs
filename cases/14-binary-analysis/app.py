#!/usr/bin/env python3
"""Servicio del caso 14 — Análisis de binarios de terceros.
...
PORT = int(os.environ.get("SANDBOX_PORT", "8814"))
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path in ("/health", "/api/status"):
            self._json(200, {"status": "ok", "case": "14", "runtime": RUNTIME})
            return
        if self.path == "/":
            self._json(200, {
                "case": "14",
                "title": "Análisis de binarios de terceros",
                "teaches": "Un binario no se puede leer. La única forma de saber qué hace es ejecutarlo donde actuar no tenga consecuencias.",
                "runtime": RUNTIME,
...
def main() -> None:
    if SOCKET_PATH:
        ...
        with UnixServer(SOCKET_PATH, Handler) as server:
            ...
            print(f"servicio 14-binary-analysis en unix:{SOCKET_PATH} · runtime={RUNTIME} · sin red", flush=True)
            server.serve_forever()
        return

    socketserver.TCPServer.allow_reuse_address = True
    with socketserver.TCPServer(("127.0.0.1", PORT), Handler) as server:
        print(f"servicio 14-binary-analysis en 127.0.0.1:{PORT} · runtime={RUNTIME}", flush=True)
        server.serve_forever()
