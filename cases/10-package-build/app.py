#!/usr/bin/env python3
"""Servicio del caso 10 — Construcción de paquetes de terceros.
...
PORT = int(os.environ.get("SANDBOX_PORT", "8810"))
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path in ("/health", "/api/status"):
            self._json(200, {"status": "ok", "case": "10", "runtime": RUNTIME})
            return
        if self.path == "/":
            self._json(200, {
                "case": "10",
                "title": "Construcción de paquetes de terceros",
                "teaches": "Compilar no necesita red y casi siempre la tiene. Cerrarla tras verificar es un control barato que casi nadie aplica.",
                "runtime": RUNTIME,
...
def main() -> None:
    if SOCKET_PATH:
        ...
        with UnixServer(SOCKET_PATH, Handler) as server:
            ...
            print(f"servicio 10-package-build en unix:{SOCKET_PATH} · runtime={RUNTIME} · sin red", flush=True)
            server.serve_forever()
        return

    socketserver.TCPServer.allow_reuse_address = True
    with socketserver.TCPServer(("127.0.0.1", PORT), Handler) as server:
        print(f"servicio 10-package-build en 127.0.0.1:{PORT} · runtime={RUNTIME}", flush=True)
        server.serve_forever()
