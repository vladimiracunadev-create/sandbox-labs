#!/usr/bin/env python3
"""Servicio del caso 08 — Sandbox de herramientas de agente IA.
...
PORT = int(os.environ.get("SANDBOX_PORT", "8808"))
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path in ("/health", "/api/status"):
            self._json(200, {"status": "ok", "case": "08", "runtime": RUNTIME})
            return
        if self.path == "/":
            self._json(200, {
                "case": "08",
                "title": "Sandbox de herramientas de agente IA",
                "teaches": "Lo que el agente lee es dato, nunca orden. El mediador vive fuera de su proceso, o el aislamiento es decorativo.",
                "runtime": RUNTIME,
...
def main() -> None:
    if SOCKET_PATH:
        ...
        with UnixServer(SOCKET_PATH, Handler) as server:
            ...
            print(f"servicio 08-ai-agent-tools en unix:{SOCKET_PATH} · runtime={RUNTIME} · sin red", flush=True)
            server.serve_forever()
        return

    socketserver.TCPServer.allow_reuse_address = True
    with socketserver.TCPServer(("127.0.0.1", PORT), Handler) as server:
        print(f"servicio 08-ai-agent-tools en 127.0.0.1:{PORT} · runtime={RUNTIME}", flush=True)
        server.serve_forever()
