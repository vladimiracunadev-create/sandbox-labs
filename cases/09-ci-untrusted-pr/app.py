#!/usr/bin/env python3
"""Servicio del caso 09 — Runner de CI con pull request externo.
...
PORT = int(os.environ.get("SANDBOX_PORT", "8809"))
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path in ("/health", "/api/status"):
            self._json(200, {"status": "ok", "case": "09", "runtime": RUNTIME})
            return
        if self.path == "/":
            self._json(200, {
                "case": "09",
                "title": "Runner de CI con pull request externo",
                "teaches": "La etapa que ejecuta código ajeno no tiene llaves, y la que tiene llaves no ejecuta código ajeno.",
                "runtime": RUNTIME,
...
def main() -> None:
    if SOCKET_PATH:
        ...
        with UnixServer(SOCKET_PATH, Handler) as server:
            ...
            print(f"servicio 09-ci-untrusted-pr en unix:{SOCKET_PATH} · runtime={RUNTIME} · sin red", flush=True)
            server.serve_forever()
        return

    socketserver.TCPServer.allow_reuse_address = True
    with socketserver.TCPServer(("127.0.0.1", PORT), Handler) as server:
        print(f"servicio 09-ci-untrusted-pr en 127.0.0.1:{PORT} · runtime={RUNTIME}", flush=True)
        server.serve_forever()
