#!/usr/bin/env python3
"""Servicio del caso 15 — Instalación de cadena de suministro.
...
PORT = int(os.environ.get("SANDBOX_PORT", "8815"))
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path in ("/health", "/api/status"):
            self._json(200, {"status": "ok", "case": "15", "runtime": RUNTIME})
            return
        if self.path == "/":
            self._json(200, {
                "case": "15",
                "title": "Instalación de cadena de suministro",
                "teaches": "Instalar ejecuta scripts de cientos de personas antes de que hayas escrito una línea. El entorno vacío convierte «se llevó tu token» en «buscó y no había».",
                "runtime": RUNTIME,
...
def main() -> None:
    if SOCKET_PATH:
        ...
        with UnixServer(SOCKET_PATH, Handler) as server:
            ...
            print(f"servicio 15-supply-chain en unix:{SOCKET_PATH} · runtime={RUNTIME} · sin red", flush=True)
            server.serve_forever()
        return

    socketserver.TCPServer.allow_reuse_address = True
    with socketserver.TCPServer(("127.0.0.1", PORT), Handler) as server:
        print(f"servicio 15-supply-chain en 127.0.0.1:{PORT} · runtime={RUNTIME}", flush=True)
        server.serve_forever()
