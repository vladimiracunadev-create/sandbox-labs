#!/usr/bin/env python3
"""Servicio del caso 11 — Renderizado de documentos.
...
PORT = int(os.environ.get("SANDBOX_PORT", "8811"))
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path in ("/health", "/api/status"):
            self._json(200, {"status": "ok", "case": "11", "runtime": RUNTIME})
            return
        if self.path == "/":
            self._json(200, {
                "case": "11",
                "title": "Renderizado de documentos",
                "teaches": "Aquí lo desconocido son los datos y el código es tuyo. Hay que aislar tu propio software, porque el fallo va a estar ahí.",
                "runtime": RUNTIME,
...
def main() -> None:
    if SOCKET_PATH:
        ...
        with UnixServer(SOCKET_PATH, Handler) as server:
            ...
            print(f"servicio 11-document-render en unix:{SOCKET_PATH} · runtime={RUNTIME} · sin red", flush=True)
            server.serve_forever()
        return

    socketserver.TCPServer.allow_reuse_address = True
    with socketserver.TCPServer(("127.0.0.1", PORT), Handler) as server:
        print(f"servicio 11-document-render en 127.0.0.1:{PORT} · runtime={RUNTIME}", flush=True)
        server.serve_forever()
