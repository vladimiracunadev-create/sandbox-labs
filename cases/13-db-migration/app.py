#!/usr/bin/env python3
"""Servicio del caso 13 — Migraciones de base de datos.
...
PORT = int(os.environ.get("SANDBOX_PORT", "8813"))
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path in ("/health", "/api/status"):
            self._json(200, {"status": "ok", "case": "13", "runtime": RUNTIME})
            return
        if self.path == "/":
            self._json(200, {
                "case": "13",
                "title": "Migraciones de base de datos",
                "teaches": "Casi todo se puede probar ejecutándolo otra vez. Una migración se ejecuta una vez sobre datos que existen una vez.",
                "runtime": RUNTIME,
...
def main() -> None:
    if SOCKET_PATH:
        ...
        with UnixServer(SOCKET_PATH, Handler) as server:
            ...
            print(f"servicio 13-db-migration en unix:{SOCKET_PATH} · runtime={RUNTIME} · sin red", flush=True)
            server.serve_forever()
        return

    socketserver.TCPServer.allow_reuse_address = True
    with socketserver.TCPServer(("127.0.0.1", PORT), Handler) as server:
        print(f"servicio 13-db-migration en 127.0.0.1:{PORT} · runtime={RUNTIME}", flush=True)
        server.serve_forever()
